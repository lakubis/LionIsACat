'''
First made: 17.11.2023
Last revised: 17.11.2023
Retype code base on PEP-8 Style
'''
...
#%%
# Function to calculate number of alive motorist
def alive_num(model):
    num_of_alive = 0
...
class battery(Agent):
    #Benchmark from phone batery. After 800 charging cycle, battery health will drop to 80%
    degradation_rate = 0.00025 

    def __init__(self, unique_id, model, max_cap=2600, health=1):
        super().__init__(unique_id,model)
        
        # Battery has 3 parameters that is:
        # max_cap: maximum battery capacity, in Watt-hour unit (2 Gogoro batteries 1.3 kWh)
        # health: battery health where, real_cap = max_cap * health. Health ranging from 0 to 1. By default 1.
        # charge: battery current capacity, charge < real_cap. Battery always full when initialize
    ...
    def degrade(self):
        self.health -= battery.degradation_rate #Each battery has a uniform degredation rate 
        #To reflect the battery capacity
    ...
    def consume_charge(self):
        ...
        if self.charge < 0:
            raise Exception("Charge can't have negative value")
...
class motorist(Agent):
    ...
    def __init__(self, unique_id, pos, model, status, batteries=None, moore=False): 
        ...
        #Motorist name and status (between commuter, taxi, or noise)
    ...
    def change_battery(self):
        ...
        #Checking each station with inventory or not
        if self.target_station.inventory_size > 0:
            #Checking station has fully charged battery or not:
            ...
            elif (len(self.target_station.inventory_full) == 0):#If failed to charge
    ...
    def move_to_station(self):
        ...
        if abs(self.pos[0]-self.target_station.pos[0]) > 0:
            ...
            raise Exception("Still in location but still asked to move")
    ...
    def set_target_station(self):
        ...
        for stat in stats:
            ...
            for stat in stats:
                new_man_distance = abs(self.pos[0]-stat.pos[0])
                + abs(self.pos[1]-stat.pos[1])
...
class station(Agent):
    ...
    '''
    def __init__(
        self, unique_id, pos, model, inventory_size=40, 
        charging_port_size=10, assigned_batteries=None
        ):
        super().__init__(unique_id, model)
